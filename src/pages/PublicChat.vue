<script>
import AppH1 from '../components/AppH1.vue';

export default {
    name: 'PublicChat',
    components: { AppH1, },
    data(){
        return{
            messages: [
                {
                    id: 1,
                    email: 'Marlos@gmail.com',
                    content: 'Hola Mundo!',
                    created_at : new Date(),
                },
                {
                    id: 2,
                    email: 'Carlos@gmail.com',
                    content: 'Hola soy Carlos!',
                    created_at : new Date(),
                },
                {
                    id: 3,
                    email: 'Warlos@gmail.com',
                    content: 'Soy Warlos!',
                    created_at : new Date(),
                },
            ],
            newMessage: {
                email: '',
                content: '',
            }
        }
    },
    methods: {
        handleSubmit(){
            this.messages.push({
                id: this.messages.length,
                email: this.newMessage.email,
                content: this.newMessage.content,
                created_at: new Date(),
            });
            this.newMessage.content = "";
        }
    }
}
</script>

<template>
    <AppH1>Chat global</AppH1>

    <div class="flex gap-4">
        <section class="w-9/12 h-100 max-h-[400px] overflow-y-auto p-4 border border-gray-200 rounded">
            <!-- max-h-[400px]: Altura máxima de 400px. Puedes ajustarla a lo que necesites. 
             overflow-y-auto: Activa scroll vertical cuando el contenido excede el tamaño. -->
            <h2 class="sr-only">Lista de Mensajes</h2>
            <ol>
                <li v-for="message in messages"
                :key="message.id"
                class="p-2 bg-gray-100">
                    <div class="mb-1"><span class="font-bold">{{ message.email }} </span> dijo:</div>
                    <div class="mb-1">{{ message.content }}</div>
                    <div class="mb-1 text-sm text-gray-700">{{ message.created_at }}</div>
                </li>
            </ol>
        </section>
        <section>
            <h2 class="mb-4 text-xl">Enviar un Mensaje</h2>
            <form action="#"
            @submit.prevent="handleSubmit">
                    <div class="mb-4">
                        <label for="email">Email: </label>
                        <input
                         type="email" 
                         id="email" 
                         class=" w-full p-2 border border-gray-400"
                         v-model="newMessage.email">
                    </div>
                    <div class="mb-4">
                        <label for="content">Mensaje: </label>
                        <textarea 
                            name="content"
                            id="content"
                            class="border w-full p-2 border-gray-400"
                            v-model="newMessage.content">
                        </textarea>
                    </div>
                    <button type="submit"
                    class="bg-blue-600 text-white py-2 px-4 rounded cursor-pointer transition duration-200 hover:bg-blue-500
                     ">Enviar</button>
                </form>
        </section>
    </div>
</template>